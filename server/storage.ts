...
export interface IStorage {
  ...
  // The MemStorage class is not used in production but kept for completeness
  export class MemStorage implements IStorage {
      // ... (rest of the MemStorage class is unchanged)
      private users: Map<string, User> = new Map();
      private analogies: Map<string, Analogy> = new Map();
      constructor() { /* ... */ }
      ...
      async upsertUser(userData: UpsertUser): Promise<User> {
        ...
        const user: User = { id: userData.id || randomUUID(), email: userData.email || null, firstName: userData.firstName || null, lastName: userData.lastName || null, profileImageUrl: userData.profileImageUrl || null, personalizationInterests: userData.personalizationInterests || [], defaultKnowledgeLevel: userData.defaultKnowledgeLevel || "intermediate", analogyStyle: userData.analogyStyle || "conversational", saveHistory: userData.saveHistory ?? true, createdAt: new Date(), updatedAt: new Date(), };
      ...
      async createUser(insertUser: InsertUser): Promise<User> {
        const id = randomUUID();
        const user: User = { id, email: insertUser.email || null, firstName: insertUser.firstName || null, lastName: insertUser.lastName || null, profileImageUrl: insertUser.profileImageUrl || null, createdAt: new Date(), updatedAt: new Date(), personalizationInterests: insertUser.personalizationInterests || [], defaultKnowledgeLevel: insertUser.defaultKnowledgeLevel || "intermediate", analogyStyle: insertUser.analogyStyle || "conversational", saveHistory: insertUser.saveHistory ?? true, };
      ...
      async getAnalogy(id: string): Promise<Analogy | undefined> { return this.analogies.get(id); }
      async createAnalogy(insertAnalogy: InsertAnalogy): Promise<Analogy> {
        const id = randomUUID();
        const analogy: Analogy = { ...insertAnalogy, id, createdAt: new Date(), isFavorite: false, personalizationInterests: insertAnalogy.personalizationInterests || [], userInputContext: insertAnalogy.userInputContext || null, modelUsed: insertAnalogy.modelUsed || "gpt-4o", };
        this.analogies.set(id, analogy);
        return analogy;
      }
      async getUserAnalogies(userId: string, limit = 50, offset = 0): Promise<Analogy[]> {
        const userAnalogies = Array.from(this.analogies.values()).filter(analogy => analogy.userId === userId).sort((a, b) => { const aTime = a.createdAt ? new Date(a.createdAt).getTime() : 0; const bTime = b.createdAt ? new Date(b.createdAt).getTime() : 0; return bTime - aTime; }).slice(offset, offset + limit);
      ...
      async deleteAnalogy(id: string): Promise<boolean> { return this.analogies.delete(id); }
  }
